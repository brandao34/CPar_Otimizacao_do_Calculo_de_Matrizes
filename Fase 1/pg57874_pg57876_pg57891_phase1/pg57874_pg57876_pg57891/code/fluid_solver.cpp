...
void lin_solve(int M, int N, int O, int b, float *x, float *x0, float a, float c) {
  int blockSize = 6 ; // Tamanho do bloco
  // localidade espacial, linha de cache 
  for (int l = 0; l < LINEARSOLVERTIMES; l++) {
  
  // i j k ?? 

    // floop unrollinh ? 
    for (int kBlock = 1; kBlock <= M; kBlock += blockSize) {
      for (int jBlock = 1; jBlock <= N; jBlock += blockSize) {
        for (int iBlock = 1; iBlock <= O; iBlock += blockSize) {

          // Processar o bloco
          for (int k = kBlock; k < kBlock + blockSize && k <= M; k++) {
            for (int j = jBlock; j < jBlock + blockSize && j <= N; j++) {
              for (int i = iBlock; i < iBlock + blockSize && i <= O; i++) {
          // 20 * M * M * M 
          // Acesso aos índices na ordem inversa para melhorar a localidade
        ...
        // se K e J forem muito grandes, pode levar i ser perdedio 
        }
      }
    }
        }
    ...
    // O 1 ciclo for implica que esta funcao é chamada 20 vezes
    set_bnd(M, N, O, b, x);
  }
}
